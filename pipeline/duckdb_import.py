...
def import_csv_to_duckdb(input_files: list, database_connection: object, file_directory: str):
    ...
    for f in input_files:
        ...
        if table_name is not None:
            try:
                database_connection.sql(f"""COPY raw.{table_name} FROM '{filename}' (HEADER, DELIMITER ',');""")
...
if __name__ == "__main__":
    ...
    logger.info(f"{len(all_input_files)} files successfully imported from {outputdir_clean}")
